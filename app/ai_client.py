...
class AIClient:
    ...
    async def chat(self, prompt: str, max_tokens: int = 1024, timeout: float = 300.0, json_mode: bool = False, json_schema: dict | None = None) -> str:
        service = f"ai:{self.provider}"
        if self._breaker.is_open(service):
            raise RuntimeError(f"Circuit breaker open for {service}")
        try:
            result = await asyncio.wait_for(
                self._chat_with_retry(prompt, max_tokens, json_mode=json_mode, **({"json_schema": json_schema} if json_schema else {})),
                timeout=timeout,
            )
    ...
    @_ai_retry
    async def _chat_with_retry(self, prompt: str, max_tokens: int, json_mode: bool = False, json_schema: dict | None = None) -> str:
        if self.provider == "anthropic":
            return await self._anthropic_chat(prompt, max_tokens)
        elif self.provider == "bedrock":
            return await self._bedrock_chat(prompt, max_tokens)
        elif self.provider == "ollama":
            return await self._ollama_chat(prompt, max_tokens, json_mode=json_mode, **({"json_schema": json_schema} if json_schema else {}))
    ...
    async def _ollama_chat(self, prompt: str, max_tokens: int, json_mode: bool = False, json_schema: dict | None = None) -> str:
